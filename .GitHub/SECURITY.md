...
TwoAuth operates entirely within the user's browser. It is a statically-exported application (`output: 'export'` in Next.js), which means there is **no active Node.js server running in production**. All data processing, encryption, and storage happens locally on the user's device.
