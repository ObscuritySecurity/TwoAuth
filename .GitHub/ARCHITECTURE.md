...
TwoAuth is a statically-exported Next.js application (`output: 'export'`). This is the most critical architectural decision: **there is no active Node.js server running in production.** All operations—including data storage, encryption, decryption, and TOTP code generation—occur exclusively within the user's browser on their local device.
