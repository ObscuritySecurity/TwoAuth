...
import { BiometricAuth, Credential } from '@aparajita/capacitor-biometric-auth';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    async function checkBiometrics() {
      if (Capacitor.isPluginAvailable('BiometricAuth')) {
        ...
        setIsNativeBiometricAvailable(available);
      }
    }
    checkBiometrics();
  }, []);
  ...
  const unregisterBiometrics = async () => {
      try {
        await BiometricAuth.deleteCredential({
            server: window.location.hostname
        });
      } catch (e) {
          console.error("Could not delete credentials, they might not have existed.", e)
      } finally {
